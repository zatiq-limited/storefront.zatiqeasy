...
const ContactForm1 = ({ settings }: ContactForm1Props) => {
  const {
  ...
  const headerMarginClass = viewMode
    ? showMobileLayout
      ? 'mb-10'
      : 'mb-12'
    : 'mb-10 md:mb-12';
  ...
  const textareaRoundedClass = viewMode
    ? showMobileLayout
      ? 'rounded-2xl'
      : 'rounded-3xl'
      ...
      <div className={`container mx-auto ${containerPaddingClass}`}>
        ...
        {(title || subtitle) && (
          <div className={`text-center ${headerMarginClass}`}>
            {subtitle && <p className={`text-gray-500 ${subtitleTextClass} mb-2`}>{subtitle}</p>}
            {title && (
              <h2 className={`${titleSizeClass} font-bold`} style={{ color: textColor }}>
                {title}
              </h2>
            )}
          </div>
        ...
        {status === 'success' ? (
          <div className="max-w-2xl mx-auto text-center py-12">
            ...
            <button
              ...
              className={`${buttonPaddingClass} ${buttonTextClass} rounded-full font-medium text-white transition-opacity hover:opacity-90`}
          ...
          <form onSubmit={handleSubmit} className="max-w-5xl mx-auto">
                ...
                {otherFields.map((field) => (
                  <div key={field.id}>
                    <input
                      ...
                      className={`w-full ${inputPaddingClass} rounded-full border border-gray-200 bg-white focus:outline-none focus:border-gray-400 transition-colors ${inputTextClass}`}
              ...
              <div className="flex flex-col">
                {textareaFields.map((field) => (
                  <textarea
                    ...
                    value={formData[field.settings.name || ''] || ''}
                    onChange={handleChange}
                    className={`w-full h-full min-h-[200px] ${textareaPaddingClass} ${textareaRoundedClass} border border-gray-200 bg-white focus:outline-none focus:border-gray-400 transition-colors ${inputTextClass} resize-none`}
                    style={{ color: textColor }}
                  />
                ))}
              </div>
            ...
            {status === 'error' && errorMessage && (
              <div className={`mt-4 p-4 bg-red-50 border border-red-200 ${textareaRoundedClass} flex items-center gap-2`}>
            ...
            <div className="flex justify-end mt-6">
              <button
                ...
                className={`${buttonPaddingClass} ${buttonTextClass} rounded-full text-white font-medium transition-all duration-200 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2`}
                style={{ backgroundColor: submitButtonColor }}
              >
                {status === 'loading' ? (
                  <>
