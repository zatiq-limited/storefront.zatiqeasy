/**
 * ProductCard16 - Elegant with Border and Centered Content
...
export default function ProductCard16({
  ...
  return (
    <div className="bg-white border-2 border-gray-100 rounded-none overflow-hidden hover:border-black transition-all duration-300 cursor-pointer">
      <Link href={`/products/${handle}`} className="block">
        ...
        <div className="relative h-40 sm:h-52 lg:h-60">
          ...
          {/* Out of Stock Badge */}
          {isOutOfStock && (
            <div className="absolute top-2 left-2 sm:top-3 sm:left-3 px-1.5 py-0.5 sm:px-2 sm:py-1 text-[10px] sm:text-xs bg-gray-600 text-white font-medium">
              Out of Stock
            </div>
          )}
          {/* Mobile Icon Buttons */}
          <div className="absolute bottom-2 right-2 flex gap-1.5 sm:hidden">
            {quickAddEnabled && (
              <button
                className={`w-7 h-7 rounded flex items-center justify-center shadow-md transition-opacity ${
                  isOutOfStock
            ...
            {buyNowEnabled && !isOutOfStock && (
              <button
                className="w-7 h-7 rounded flex items-center justify-center shadow-md hover:opacity-90 transition-opacity"
        ...
        <div className="p-3 sm:p-5 text-center border-t border-gray-100">
          {/* Vendor */}
          {vendor && (
          ...
          {/* Title */}
          <h3 className="text-xs sm:text-sm font-medium text-gray-900 mb-1 sm:mb-2 line-clamp-1">
            {title}
          </h3>
          ...
          <div className="hidden sm:flex gap-2 justify-center">
            {quickAddEnabled && (
              <>
                {isOutOfStock ? (
                  <button
                    className="px-4 h-9 sm:px-6 sm:h-10 text-xs sm:text-sm font-medium cursor-not-allowed opacity-60"
                  ...
                  <div
                    className="h-9 sm:h-10 border flex items-center justify-between overflow-hidden"
                  ...
                  <button
                    className="px-4 h-9 sm:px-6 sm:h-10 text-xs sm:text-sm font-medium transition-opacity hover:opacity-90"
                    style={{ backgroundColor: buttonBgColor, color: buttonTextColor }}
                    onClick={(e) => {
                      e.preventDefault();
                      onAddToCart?.();
            ...
            {buyNowEnabled && !isOutOfStock && !isInCart && (
              <button
                className="px-4 h-9 sm:px-6 sm:h-10 text-xs sm:text-sm font-medium border-2 transition-all hover:opacity-80"
