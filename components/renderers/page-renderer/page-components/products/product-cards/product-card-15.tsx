/**
 * ProductCard15 - Modern with Inner Rounded Image
    ...
    <div className="bg-gray-50 rounded-xl overflow-hidden hover:bg-white hover:shadow-xl transition-all duration-300 cursor-pointer">
      <Link href={`/products/${handle}`} className="block">
        ...
        <div className="relative h-[140px] sm:h-[200px] m-2 sm:m-3 rounded-lg overflow-hidden">
          ...
          {isOutOfStock ? (
            <div className="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 px-1.5 py-0.5 sm:px-2 sm:py-0.5 text-[10px] sm:text-xs bg-gray-600 text-white rounded">
              Out of Stock
            </div>
          ) : (
            badge && (
              <div className="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 px-1.5 py-0.5 sm:px-2 sm:py-0.5 text-[10px] sm:text-xs bg-black text-white rounded">
                {badge}
              </div>
            )
          )}
          ...
          <div className="hidden sm:flex gap-2">
            {quickAddEnabled && (
              <>
                {isOutOfStock ? (
                  <button
                    className="flex-1 h-9 lg:h-10 text-xs lg:text-sm rounded-lg font-medium cursor-not-allowed opacity-60"
                  ...
                  <div
                    className="flex-1 h-9 lg:h-10 border rounded-lg flex items-center justify-between overflow-hidden"
