...
export function GripNavbar() {
  ...
  return (
    <>
      {/* Message on Top */}
      {messageOnTop && (
        <div className="bg-gradient-to-r from-blue-600 to-landing-primary text-white text-center py-2.5 px-4 text-sm font-medium">
          {messageOnTop}
        </div>
      )}
              ...
              {shopDetails?.image_url ? (
                ...
                <Link href={baseUrl || "/"} className="group flex items-center gap-3">
                  <div className="flex flex-col">
                    <span className="font-bold text-xl text-blue-zatiq group-hover:text-blue-zatiq/80 transition-colors dark:text-white">
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="sticky inset-x-0 top-16 md:hidden bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-lg z-40">
          <div className="p-4 space-y-4 text-center">
            <Link
              href={`${baseUrl}/products`}
              className="block py-2 text-gray-600 hover:text-blue-zatiq transition-colors dark:text-gray-300 dark:hover:text-white"
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="bg-white text-blue-zatiq text-xs font-bold rounded-full px-2 py-0.5">
                  {cartCount}
                </span>
              )}
            </Link>
