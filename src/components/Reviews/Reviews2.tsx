...
export default function Reviews2({ settings, blocks, pageData }: Reviews2Props) {
  const swiperRef = useRef<any>(null);
  const [isReversed, setIsReversed] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentIndex, setCurrentIndex] = useState(0);

  const testimonials = settings?.reviews || defaultReviews;
  ...
  }, [isReversed, isPaused, currentIndex, testimonials.length]);
