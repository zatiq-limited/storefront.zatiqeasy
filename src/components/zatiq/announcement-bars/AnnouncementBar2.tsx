import React from "react";

interface AnnouncementBar2Props {
  phone?: string;
  message?: string;
  // Support both naming conventions (camelCase and snake_case)
  backgroundColor?: string;
  background_color?: string;
  textColor?: string;
  text_color?: string;
  showSocial?: boolean;
  show_social?: boolean;
  show_phone?: boolean;
  // Individual social toggles
  show_instagram?: boolean;
  show_youtube?: boolean;
  show_facebook?: boolean;
  show_twitter?: boolean;
  // Individual social URLs
  instagram_url?: string;
  youtube_url?: string;
  facebook_url?: string;
  twitter_url?: string;
  // Legacy socialLinks object
  socialLinks?: {
    instagram?: string;
    youtube?: string;
    facebook?: string;
    twitter?: string;
  };
  dismissible?: boolean;
  sticky?: boolean;
}

const AnnouncementBar2: React.FC<AnnouncementBar2Props> = ({
  phone,
  message,
  backgroundColor,
  background_color,
  textColor,
  text_color,
  showSocial,
  show_social,
  show_phone = true,
  show_instagram,
  show_youtube,
  show_facebook,
  show_twitter,
  instagram_url,
  youtube_url,
  facebook_url,
  twitter_url,
  socialLinks = {},
  dismissible,
  sticky,
}) => {
  // Resolve props (support both naming conventions)
  const bgColor = background_color || backgroundColor || '#23856D';
  const txtColor = text_color || textColor || '#FFFFFF';
  const shouldShowSocial = show_social ?? showSocial ?? true;

  // Build social links from individual props or legacy object
  const resolvedSocialLinks = {
    instagram: instagram_url || socialLinks.instagram,
    youtube: youtube_url || socialLinks.youtube,
    facebook: facebook_url || socialLinks.facebook,
    twitter: twitter_url || socialLinks.twitter,
  };

  // Check if individual social icons should show
  const showInstagram = show_instagram ?? !!resolvedSocialLinks.instagram;
  const showYoutube = show_youtube ?? !!resolvedSocialLinks.youtube;
  const showFacebook = show_facebook ?? !!resolvedSocialLinks.facebook;
  const showTwitter = show_twitter ?? !!resolvedSocialLinks.twitter;
  const [isVisible, setIsVisible] = React.useState(true);

  if (!isVisible) return null;

  return (
    <div
      className={`min-h-10 sm:min-h-14 flex items-center justify-between py-1.5 sm:py-3 px-4 ${
        sticky ? "sticky top-0 z-50" : ""
      }`}
      style={{
        backgroundColor: bgColor,
        color: txtColor,
      }}
    >
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-0 sm:gap-6 lg:gap-10">
        {show_phone && phone && (
          <div className="flex items-center gap-2">
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_804_31797)">
                <path
                  d="M3.65447 1.32829C3.59544 1.25236 3.52092 1.18985 3.43587 1.14492C3.35082 1.09999 3.25719 1.07368 3.16119 1.06772C3.06519 1.06176 2.96902 1.07629 2.87907 1.11036C2.78912 1.14442 2.70744 1.19724 2.63947 1.26529L1.60547 2.30029C1.12247 2.78429 0.944471 3.4693 1.15547 4.0703C2.03121 6.55788 3.45577 8.81639 5.32347 10.6783C7.1854 12.546 9.4439 13.9705 11.9315 14.8463C12.5325 15.0573 13.2175 14.8793 13.7015 14.3963L14.7355 13.3623C14.8035 13.2943 14.8563 13.2127 14.8904 13.1227C14.9245 13.0327 14.939 12.9366 14.933 12.8406C14.9271 12.7446 14.9008 12.6509 14.8558 12.5659C14.8109 12.4808 14.7484 12.4063 14.6725 12.3473L12.3655 10.5533C12.2843 10.4904 12.19 10.4467 12.0895 10.4256C11.989 10.4044 11.8851 10.4064 11.7855 10.4313L9.59547 10.9783C9.30315 11.0514 8.99689 11.0475 8.70652 10.967C8.41614 10.8866 8.15154 10.7324 7.93847 10.5193L5.48247 8.06229C5.26924 7.84933 5.1148 7.58478 5.03418 7.29439C4.95357 7.00401 4.94954 6.6977 5.02247 6.4053L5.57047 4.2153C5.59538 4.11569 5.59734 4.01173 5.57621 3.91126C5.55507 3.81079 5.51139 3.71642 5.44847 3.63529L3.65447 1.32829ZM1.88447 0.511295C2.05947 0.336244 2.2697 0.200425 2.50121 0.112856C2.73272 0.0252871 2.98021 -0.0120282 3.22725 0.00338804C3.47429 0.0188043 3.71522 0.0865995 3.93405 0.202272C4.15288 0.317944 4.3446 0.478846 4.49647 0.674295L6.29047 2.98029C6.61947 3.4033 6.73547 3.9543 6.60547 4.4743L6.05847 6.6643C6.03019 6.77772 6.03172 6.89654 6.06291 7.0092C6.0941 7.12187 6.15388 7.22456 6.23647 7.3073L8.69347 9.76429C8.77631 9.84705 8.87916 9.90694 8.99202 9.93813C9.10488 9.96933 9.22389 9.97076 9.33747 9.9423L11.5265 9.3953C11.7831 9.33113 12.0509 9.32615 12.3098 9.38072C12.5686 9.43529 12.8116 9.54799 13.0205 9.71029L15.3265 11.5043C16.1555 12.1493 16.2315 13.3743 15.4895 14.1153L14.4555 15.1493C13.7155 15.8893 12.6095 16.2143 11.5785 15.8513C8.93965 14.9228 6.54374 13.4121 4.56847 11.4313C2.58776 9.45631 1.07708 7.06075 0.148471 4.4223C-0.213529 3.3923 0.111471 2.2853 0.851471 1.54529L1.88547 0.511295H1.88447Z"
                  fill="white"
                />
              </g>
              <defs>
                <clipPath id="clip0_804_31797">
                  <rect width="16.0005" height="16.0004" fill="white" />
                </clipPath>
              </defs>
            </svg>

            <span className="text-sm font-bold">{phone}</span>
          </div>
        )}

        <div className="flex items-center gap sm:gap-6 lg:gap-10">
          <div className="flex-1 text-center">
            <p className="text-sm font-bold">{message}</p>
          </div>

          {shouldShowSocial && (
            <div className="hidden sm:flex items-center gap-3">
              <span className="text-sm font-bold">Follow Us :</span>
              {showInstagram && resolvedSocialLinks.instagram && (
                <a
                  href={resolvedSocialLinks.instagram}
                  className="hover:opacity-80 transition-opacity"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                  </svg>
                </a>
              )}
              {showYoutube && resolvedSocialLinks.youtube && (
                <a
                  href={resolvedSocialLinks.youtube}
                  className="hover:opacity-80 transition-opacity"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                  </svg>
                </a>
              )}
              {showFacebook && resolvedSocialLinks.facebook && (
                <a
                  href={resolvedSocialLinks.facebook}
                  className="hover:opacity-80 transition-opacity"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
              )}
              {showTwitter && resolvedSocialLinks.twitter && (
                <a
                  href={resolvedSocialLinks.twitter}
                  className="hover:opacity-80 transition-opacity"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                  </svg>
                </a>
              )}
            </div>
          )}
          {dismissible && (
            <button
              onClick={() => setIsVisible(false)}
              className="ml-4 hover:opacity-70 transition-opacity"
              aria-label="Close announcement"
            >
              ✕
            </button>
          )}
        </div>
      </div>
    </div>
  );
};

export default AnnouncementBar2;
