    ...
    <div className="py-6 md:py-8">
          ...
          <>
            ...
            {visiblePages[0] > 1 && (
              <>
                <button
                  ...
                  className="min-w-[2.5rem] h-10 px-2 sm:px-3 border border-border rounded-lg hover:bg-muted/50 transition-all text-sm sm:text-base font-medium text-foreground shadow-sm hover:border-border"
                >
                  1
                </button>
                {visiblePages[0] > 2 && (
                  <span className="px-1 sm:px-2 text-muted-foreground select-none">...</span>
                )}
              </>
            )}
            ...
            {visiblePages.map((page) => (
              <button
                ...
                className={`min-w-[2.5rem] h-10 px-2 sm:px-3 rounded-lg font-medium transition-all text-sm sm:text-base ${
                  page === current_page
      ...
      <div className="mt-4 text-center">
        <p className="text-xs sm:text-sm text-muted-foreground">
          Showing <span className="font-semibold text-foreground">{pagination.from}-{pagination.to}</span> of <span className="font-semibold text-foreground">{pagination.total}</span> products
        </p>
      </div>
    </div>
  );
