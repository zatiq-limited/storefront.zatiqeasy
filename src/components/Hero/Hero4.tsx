...
export default function Hero4({ settings, blocks, pageData }: Hero4Props) {
  const seasonTag = settings?.seasonTag || "SUMMER 2025";
  const heading = settings?.heading || "NEW COLLECTION";
  const description1 = settings?.description1 || "We know how large objects will act,";
  const description2 = settings?.description2 || "but things on a small scale.";
  const buttonText = settings?.buttonText || "SHOP NOW";
  const bgColor = settings?.bgColor || "#7FC5D9";
  const heroImage = settings?.heroImage || "/assets/hero/hero-4.png";
  const buttonColor = settings?.buttonColor || "#01B7DF";
  const buttonHoverColor = settings?.buttonHoverColor || "#26A661";

  return (
    <div 
      className="relative font-montserrat w-full max-w-[1440px] min-h-[500px] sm:min-h-[600px] lg:h-[716px] mx-auto overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 h-full flex items-center mt-24 lg:mt-0">
        <div className="px-4 md:px-20 lg:px-0 lg:ml-40 pt-8 lg:pt-8 w-full lg:w-auto">
          ...
          {/* CTA Button */}
          <button
            className="min-w-[180px] sm:min-w-56 min-h-12 sm:min-h-16 text-white font-bold uppercase transition-colors rounded text-lg sm:text-xl lg:text-2xl tracking-[0.1px] px-4 sm:px-6 leading-[28px] sm:leading-[32px]"
