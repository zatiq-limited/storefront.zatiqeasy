...
export default function Hero3({ settings, blocks, pageData }: Hero3Props) {
  const title = settings?.title || "Best Discounts 2025";
  const description = settings?.description || "Salla Store provides you with everything you need, from electronics to home furniture, along with the best discounts on products. Shop now and enjoy all the discounts on products.";
  const buttonText = settings?.buttonText || "Discover more";
  const bgColor = settings?.bgColor || "#1F2937";
  const heroImage = settings?.heroImage || "/assets/hero/hero-3.png";
  const buttonColor = settings?.buttonColor || "#3465F0";
  const buttonHoverColor = settings?.buttonHoverColor || "#2850D9";

  return (
    <div 
      className="relative w-full max-w-[1440px] min-h-[480px] sm:min-h-[558px] mx-auto overflow-hidden"
      ...
      <div className="relative px-4 md:px-6 xl:px-[200px] py-16 md:py-[136px]">
        ...
        <div className="flex items-center justify-center gap-6 md:gap-10 lg:gap-24">
          ...
          <div className="flex-1 max-w-[728px] flex flex-col items-center space-y-8">
            ...
            {/* CTA Button */}
            <button 
              className="min-w-[200px] min-h-10 sm:min-h-14 px-4 text-white font-medium text-base rounded transition-colors"
